...
int main(int argc, char *argv[])
{
    ...
    if (err != 1) error(3);
    switch(fi) {
	case 0: err = NvmeAcquireHelper(argv[2], key);
	        break;
	case 1: err = NvmeReleaseHelper(argv[2], key);
	        break;
	case 2: err = NvmeChangeHelper(argv[2], key);
	        break;
	case 3: err = NvmeResetHelper(argv[2], key);
	        break;
	case 4: err = NvmeCheckHelper(argv[2], key);
