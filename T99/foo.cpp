...
//
static
int nvme_resv_report( struct nvme_reservation_status *status, int bytes, __u32 cdw11, uint64_t &key )
{
...
//
static
int reserve_register( int fd, __u32 nsid, uint64_t key ) {
    int err;
...
//
static
int reserve_unregister( int fd, __u32 nsid, uint64_t key ) {
    int err;
...
//
static
int reserve_acquire( int fd, __u32 nsid, uint64_t key ) {
...
//
static
int reserve_release( int fd, __u32 nsid, uint64_t key ) {
    __u8 rrela = 1; //clear
...
int NvmeCheckHelper( const char *dev, uint64_t &key ) {
    ...
    if ( err == 0 ) {
        key = 0;
        err = nvme_resv_report( status, ( numd + 1 ) * 4, cdw11, key );
    } else {
       err = -1;
    }
    free( status );
...
int main() {
    __le64 payload[2] = {-1, -1};
    __u64 addr = ( __u64 )( unsigned long * ) ( payload );
    __u64 addr2 = ( __u64 ) ( payload );
    return 0;
}
